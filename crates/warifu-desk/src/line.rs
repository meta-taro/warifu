...
pub enum ToDesk {
    ...
    ///
    /// **書けるのは自分の席だけ。**どの席かは**繋いできた口で決まる**ので、
    /// ここに「誰の」は書けない —— 書けると、**同じ机の別のエージェントに化けられる。**
    ///
    /// **名乗り（どこで動いているか）はここでは変えられない。**
    /// あれは立ち上げるときに人が決める（`--as`）。**変えられると、席そのものを偽れる。**
    Profile {
        /// 表に出す名前。**空にすると消える。**
        名前: String,
        /// 短い紹介。**空にすると消える。**
...
pub enum FromDesk {
    /// 誰かが喋った。
    Heard {
        /// **通し番号。**机が刻む。既読を数えるのに使う（**D76**）
        ///
        /// **古い机は 0 を返す。**0 は「番号が無い」と読む
        #[serde(default)]
        id: u64,
        /// 誰が。**机が刻む。**繋いできた側は書けない
        from: String,
    ...
    ///
    /// **「届いた」と「読んだ」を分ける。**
    /// 届いたのは机が配った先、読んだのは**渡された**ことが確かな席だけである。
    ...
    Status {
        ...
        /// 届けた席の名。
        届いた: Vec<String>,
        /// 読んだ席の名。**渡したことが確かなものだけ。**
        読んだ: Vec<String>,
    },
    /// **席に着いた。**いつ着いたかを返す（**`issues/4`** の 1 番）。
    ///
    /// **「届いていない」と「着く前だった」を、席から見分けられるようにする。**
    /// 画面を入れ替えると机の席は全部外れる（`issues/2`）ので、
    /// **黙って繋ぎ直すと、切れている間の発言が無いことに気づけない。**
    Seated {
        /// 着いた時刻（`HH:MM`）。**この時刻より前の発言は取れない。**
        at: String,
        /// 何と呼ばれているか（画面に出る呼び方）。
        who: String,
    },
    /// **プロフィールを書いた。**どの席のものとして書いたかを返す。
    ///
    /// **「書いた」と言うなら、誰として書いたかまで言う** ——
    /// 名乗っていない席は席が決まらないので、そもそも書けない（`Denied` が返る）。
    Wrote {
        /// 誰として書いたか（画面に出る呼び方）
        who: String,
    },
...
impl ToDesk {
    ...
    pub fn 読む(行: &str) -> Result<Self, Error> {
        ...
        match &中身 {
            ...
            Self::Listen { 場所: None } => {}
            // **上限は書き手の側でも見る。**長すぎるものを机まで運ばない
            Self::Profile { 名前, 紹介 } => プロフィールを検める(名前, 紹介)?,
            // 番号だけの行。**中身が無いので検めるものが無い**
            Self::Read { .. } | Self::Status { .. } => {}
        }
        Ok(中身)
    }
