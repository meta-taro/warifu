...
fn 出している口を_数えて名前で押さえる() {
    ...
    // 2026-09-24 に pass_wait を足した —— **札は要らない**（`pass_ask` と同じ筋）。
    // **返すものは `pass_ask` と同じ 1 行**で、違うのは**待つかどうか**だけである
    // （chat_read と chat_wait を同じ札にしたのと同じ形）。
    //
    // **なぜ足したか** —— オーナー（2026-09-24・Mac Air の席ごしに）——
    // 「**押した時点であなたが検知できないということでしょう。
    //   ずーっと前から指摘しています。**」
    //
    // `pass_ask` は**いまの答えを返すだけ**なので、人が押しても
    // **エージェントには何も届かなかった** ——
    // 毎回「押しましたか」「押しました」の往復が要っていた。
    // **人が、エージェントの目の代わりをしていた。**
    //
    // **待つだけで、押させる力は持たない**（**D56** ——「札を出すのは人である」）。
    // 時間切れなら「まだ」が返る。**上限 60 秒**（`chat_wait` と同じ）。
...
fn 外へ出る引数名は_asciiだけ() {
    ...
    名を検める(
        "pass_wait",
        &schemars::schema_for!(warifu_mcp::WaitPassArgs),
    );
    名を検める(
        "room_invite",
...
#[tokio::test]
async fn 人が押した札は_立て直さずに効く() {
    // **2026-09-24 に踏んだ。**人が【通す】を押しても、**何も起きなかった。**
    //
    // ```text
    // 人が押す → pass.tsv に入る → **口は知らない** → 断られる
    // ```
    //
    // **札は口を立てるときに 1 回しか読んでいなかった。**
    // そして `pass_ask` の返りは「**次の呼びで通ります**」と言っていた ——**嘘だった。**
    //
    // オーナー ——「**通すを押しても何も起きません。
    // 何が起きたのか人には意味がわかりません。**」
    //
    // **押した棚を後から変えて、立て直さずに通ることを確かめる。**
    use std::sync::{Arc, Mutex};

    let 棚: Arc<Mutex<Vec<String>>> = Arc::new(Mutex::new(Vec::new()));
    let 読む = {
        let 棚 = Arc::clone(&棚);
        Arc::new(move || 棚.lock().expect("毒されていない").clone())
    };
    // **札を 1 つも持たせずに立てる**
    let 口 = 用意(&[]).札を読み直すには(読む);

    // **押す前は断られる**
    assert!(口.inbox_list().await.is_err(), "押す前に通りました");

    // **人が押した**（画面が pass.tsv へ書くのと同じこと）
    棚.lock()
        .expect("毒されていない")
        .push("inbox.list".to_owned());

    // **立て直していないのに通る**
    assert!(
        口.inbox_list().await.is_ok(),
        "**押したのに通りません**（立て直さないと効かない形に戻っています）"
    );

    // **押していない動作は、通らないまま**
    assert!(
        口.rules_list().await.is_err(),
        "押していない動作まで通りました"
    );
}
