...
fn 札が無くても変わったことを返す() {
    let 出た = 口()
        ...
        .expect("札は要らない");
    // **古い版を名指ししない**（2026-09-24 に落ちた）——
    // 表は**新しい 12 版だけ**を持つ回転する窓なので、
    // **タグが 13 本目を越えた時点で `v0.1.0` は落ちる。**
    // ここで見たいのは「**札なしで版の見出しが返る**」ことだけである。
    assert!(出た.contains("\n## v"), "版の見出しが無い: {出た}");
    // **「いまの版が載っている」は見ない**（2026-09-24 に自分で踏んだ）。
    //
    // この表は**タグの間の commit の見出し**を並べたものなので、
    // **版を上げてからタグを打つまでの間、いまの版はまだ表に無いのが正しい。**
    // そこを見ると、**リリースの commit そのものが通らなくなる。**
    //
    // **配るものは CI が作り直す**（`release.yml` の「版ごとの記録を作り直す」・
    // `fetch-depth: 0`）ので、**タグから建てた実行ファイルには、その版が載る。**
}
...
fn 無い版は_無いと言う() {
    ...
    // **近い版を返していないこと。**載っている版のどれかを返していたら落ちる
    assert!(!出た.contains("\n## v"), "近い版を返している: {出た}");
}
