...
//!
//! この機械（**D55**）で同じ PC の AI が喋れるようになったが、その発言は
//! **`from` に人の公開鍵を載せて飛んでいた。**
//! こちらの画面では「zumen の AI」と出るのに、**相手の画面では人が打ったのと
//! 区別が付かない。**
//!
//! この機械の中では「**繋いできた側は差出人を名乗れない。誰が言ったかはこの機械が刻む**」を
...
fn 人が言ったものには話し手が入らない() {
    // **既定は人。**札が付いていないものは、そのエージェントの人が言ったものである
    let 元 = Notice::Text {
        meeting: 会議(),
        from: 鍵([1u8; 32]),
        話し手: None,
...
#[test]
fn エージェントの持ち主は札とは別に残る() {
    // **`from` はエージェントの持ち主のまま。**札は「そのエージェントの誰が言ったか」である。
    // 混ぜると、AI が別人のエージェントから喋れることになる
    let 元 = Notice::Text {
        meeting: 会議(),
        from: 鍵([1u8; 32]),
        話し手: Some("zumen の AI".to_owned()),
