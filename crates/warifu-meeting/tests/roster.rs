//! 会議の名簿。
//!
//! ここで **4 人という上限をコードに固定する**。
//! 5 人以上にすると誰かの端末が他人の映像を中継することになり、
//! それは `decisions.md` **D7**（利用者の端末が他人の通信を中継する）そのもので、
//! 法的な決着が付いていない。**上限を「運用で気をつける」にしない。**

use warifu_core::{PublicKey, Seed};
use warifu_meeting::{Error, MAX_PARTICIPANTS, MeetingId, Notice, Roster};
...
fn 四人まで入れる() {
    ...
    assert_eq!(名簿.len(), MAX_PARTICIPANTS);
    assert!(名簿.is_full());
}

#[test]
fn 五人目は入れない() {
    // **D7 に触れない境界。**ここを緩めるのは法的整理が終わってから
    let mut 名簿 = Roster::new(鍵(1));
    名簿.add(鍵(2)).unwrap();
    名簿.add(鍵(3)).unwrap();
    名簿.add(鍵(4)).unwrap();
    ...
    assert_eq!(名簿.len(), MAX_PARTICIPANTS, "断った分が混ざっていない");
}
