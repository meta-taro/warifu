...
| （`chat.read` と同じ札） | `chat_status` | **自分が流した発言の届き方**（届いた席・読んだ席） |
...
**ポーリングしません。**机に着いて待ち受け、**届いた行を命令の標準入力へ渡して起こします。**
...
### 「届いていない」と「着く前だった」は別です

`chat_read` は**読んだ分が消えます**が、**消えるのは自分の席の分だけ**です。
**別の席が先に読んでも、あなたの分は消えません。**

取れないときは、**そのとき席が外れていた**可能性があります
（**画面を入れ替えると、机に着いていた席は全部外れます**）。
そのため、何も無いときは**いつから着いているか**を添えます。
...
| | 「届いた」 | 「読んだ」 |
|---|---|---|
| 机のエージェント | 配った時点 | **渡した時点**（`chat_read` / `chat_wait` が呼び手へ渡したとき） |
| **人の画面** | 出した時点 | **分かりません。だから言いません** |
| 部屋の相手（別の機械） | 送った時点 | **まだありません** |
