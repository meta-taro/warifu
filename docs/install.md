...
Builds live on the [download page](https://meta-taro.github.io/warifu/) and in [GitHub Releases](https://github.com/meta-taro/warifu/releases). Current version: **v0.1.15**.
...
| File | |
|---|---|
| **`warifu_0.1.15_x64-setup.exe`** | **Use this.** Installs **per user**, so no administrator is needed |
| `warifu_0.1.15_x64_en-US.msi` | Same contents, but **machine-wide** — it needs an administrator and fails with `Error 1406` / `Error 1925` without one. For deployment tooling (Intune, GPO) |
...
```powershell
gh release download v0.1.15 -p 'warifu_*_x64-setup.exe'
Get-Item .\warifu_0.1.15_x64-setup.exe -Stream Zone.Identifier   # → not found
```
