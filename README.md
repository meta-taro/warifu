...
> ⚠️ このリポジトリは設計段階です。**実装はまだありません。**方向性は `PRD.md`、未決事項は `.claude/decisions.md`、着手順は `.claude/roadmap.md` にあります。

## これで何が要らなくなるか

**サーバーを 1 台も立てず、どのクラウドにもアカウントを作らずに、4 人までのビデオ会議と文書の受け渡しをする。**

| 要らなくなるもの | 何が担うか |
|---|---|
| Teams / Meet / Zoom | warifu の会議（**4 人まで**・フルメッシュ） |
...
**5 人以上は対象外です。**SFU が必要になり、それは「利用者の端末が他人の通信を中継する」ことを
意味するため、法的な整理が付くまで着手しません（`.claude/decisions.md` D7）。
