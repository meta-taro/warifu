...
export type 会話行 = {
  ...
  who: string;
  /**
   * **顔の種。**公開鍵（全桁）か `desk:<エージェント>`。
   *
   * **連絡帳と同じ顔を出すためにある**（`contacts/Avatar.svelte`）——
   * 呼び名を付けていない相手は `who` が鍵の頭なので、
   * **`6X7BDCXBJ3DW…` と `KBN2GCQO35W…` のどちらが誰か、字では追えない**
   * （オーナー・2026-09-12）。**無ければ顔を出さない。**
   */
  顔の種?: string;
  /** 中身。 */
  body: string;
  ...
  system?: true;
  /**
   * この PC のこの機械から出た発言か（同じエージェントの AI）。
   *
  ...
  agent?: true;
  /**
   * **留守中に預かり所へ届いていた分**か（**D71**）。
   *
   * **いま届いたように見せない。**この行の時刻は「出した側の時計」であり、
   * 3 日前の言葉が、たったいま言われたように読めてしまう。
   */
  留守中?: true;
  /**
   * **自分が打った行が、どこまで行ったか**（`chat/delivery.ts`）。
   *
   * 渡した／預けた／届かない の 3 つだけ。**既読ではない** ——
   * 割符には既読を集める機械が無いので、**読んだかどうかは分からない。**
   */
  届き?: { 札: string; 訳: string };
  /**
   * いつの発言か（`HH:MM`）。
   *
...
/**
 * その行に顔を出すか。出すなら**顔の種**を返す。
 *
 * **人ではないものに顔を付けない** —— 会議からの知らせ（`system`）には出さない。
 * **自分の発言にも出さない**（自分が誰かは分かっている。行が賑やかになるだけ）。
 * **種が無ければ作らない** —— 知らない相手を、知っているように見せない。
 */
export function 差出人の顔(行: 会話行): string | null {
	if (行.system) return null;
	if (行.mine) return null;
	return 行.顔の種 && 行.顔の種 !== '' ? 行.顔の種 : null;
}
