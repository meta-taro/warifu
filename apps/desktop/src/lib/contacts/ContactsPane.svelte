<script lang="ts">
  ...
  interface Props {
     ...
     * 自分の名前を書き換えられると、**同じ機械の別のエージェントに化けられる。**
     */
    ...
    /** ルームに名前を付ける（**画面の中だけ**）。 */
    ルームに名前を付ける?: (id: string, 名前: string) => void;
    /**
     * そのルームを抜ける。
     *
    ...
    /**
     * その行の未読の数（**まだ見ていない相手の行**）。0 なら出さない。
     *
     * **既読は相手へ送らない**（`chat/unread.ts`）——
     * 出せるのは「自分がどこまで見たか」だけである。
     */
    未読?: (行: 行) => number;
  }
  const {
  ...
  const 口たち = $derived(
    相手
      ? できること({
          ...
          いま会議に居る: 相手.いま会議に居る,
          // AI の行は、その 1 つがつながっているかどうかで決まる
          この機械につながっている: 相手.いま会議に居る,
          預かり所がある,
        })
      : [],
  );
  ...
  /**
   * 次に会うときの道（`reunion.ts`）。**ほかの人を選んでいるときだけ。**
   */
  const 再会 = $derived(
    相手
      ? 再会の道を決める({
          種類: 相手.種類,
          住所を覚えている: 相手.住所を覚えている,
          鍵なしで入れる: 鍵なしで入れる.includes(相手.key),
        })
      : null,
  );
   ...
   * 行に出す名前。
   *
   * **文言の鍵を持つのは 2 つだけ** —— 自分と、誰もつながっていないときの
   * 「マイ PC エージェント」。つながっている AI は**呼び方をそのまま持っている**
  ...
  function 名(行: 行): string {
    ...
    if (名乗り) return 名乗り;
    if (行.種類 === '自分' || 行.key === この機械の印) return t(行.name as MessageKey);
  ...
  /** いまこの機械につながっているエージェントが 1 人でも居るか。 */
  ...
  /** ルームの名前を決める。**画面の中だけ。** */
  function ルームの名前を決める(行: 行) {
    const id = ルームのid(行.key);
    if (id) ルームに名前を付ける(id, 名前の下書き);
  }
  ...
  $effect(() => {
    ...
    if (!誰) return;
    if (ルームか(誰)) {
      // **付けた名前だけを入れる。**行の name は、名前が無いとき
      // `room.nth:1:1`（数えて呼ぶための印）になっている ——
      // それを欄に入れると、**生の印が人に見える**（2026-09-11 に実物で出た）
      const id = ルームのid(誰);
      名前の下書き = (id && 素材.ルームの名前?.[id]) || '';
      return;
    }
  ...
  function 名乗りを引く(行: 行): ProfileRow | undefined {
    ...
    // この機械の行の key は `desk:<呼び方>`。**つながっていないときの行（`desk:`）は素通し**
    return プロフィール.find((p) => p.who === 行.key);
  }

  /**
   * その行に「どのエージェントか」を添えるか。
   *
   * **名乗った名前と、どこで動いているかは別のものである。**
   * 名乗りだけを出すと、`git-qa` のエージェントのエージェントが「zumen」と名乗ったときに
   * **人が取り違える**（**D75**）。
   */
  function エージェントを添える(行: 行): boolean {
   ...
   * ルームは**切り詰めた文字を名前にしない** —— 名前が付いていればそれ、
   * 無ければ id の全桁（`…` は目で見るためのものである）。
   */
  function 読み上げる名(行: 行): string {
    if (ルームのid(行.key)) return 名(行);
    const エージェント = エージェントを添える(行) ? `（${エージェントの札(行)}）` : '';
    // **色だけで言わない。**読み上げにも つながっている／切れている を入れる
    const 印 = 在エージェントの印(行);
    const 在エージェント = 印 ? `（${印 === 'つながっている' ? t('presence.on') : t('presence.off')}）` : '';
    return `${名(行)}${エージェント}${在エージェント}`;
  }
  ...
  function 顔を差し替えられるか(行: 行): boolean {
    return 行.種類 === '自分' || (行.種類 === 'AI' && 行.key !== この機械の印);
  }
...
<div class="pane">
    ...
    {#each 区画 as 一区画 (一区画.title)}
      ...
      <!--
        **「マイ PC エージェント」という 1 人は居ない。**
        誰もつながっていないときに行を出すと、それが 1 人に見える ——
        ...
        >
          ...
          {:else}
            ...
            <span class="face" aria-hidden="true">
              ...
              {#if 在エージェントの印(行)}
                <span
                  class="badge {在エージェントの印(行) === 'つながっている' ? 'on' : 'off'}"
                  title={在エージェントの印(行) === 'つながっている' ? t('presence.on') : t('presence.off')}
                ></span>
              {/if}
            </span>
          {/if}
          <span class="name">{名(行)}</span>
          <!-- **まだ見ていない相手の行の数**（案 A・2026-09-11）。
               0 のときは出さない —— 0 を出すと、何も無いことが目に入り続ける -->
          {#if 未読(行) > 0}
            <span class="unread">{未読(行)}</span>
          {/if}
          <!--
            **名乗りとエージェントの両方を出す**（**D75**）。
            エージェントが `git-qa` のエージェントが「zumen」と名乗ることはできるので、
            **名前だけを見て取り違えない形**にする
          -->
          <!-- **どちらが自分のルームかを言う**（`gh issue 13`）。
               入り直すと「自分が建てたルーム」と「鍵でもらって入ったルーム」が
               2 行並ぶ。**重複ではない**が、言わないと「どちらが本物か分からない」 -->
          {#if 行.主催か}<span class="seat">{t('room.mine')}</span>{/if}
          {#if エージェントを添える(行)}<span class="seat">{エージェントの札(行)}</span>{/if}
          <!-- **在エージェントは出さない。**相手が起動しているかは分からない -->
          <!-- **在エージェントは出さない。**つながっているかどうかは、行が在ること自体で分かる -->
        </button>
      {/each}
    {/each}
  </div>
  ...
  <div class="person">
    {#if !相手}
      ...
      <section class="now">
        ...
        <dl>
          <div>
            <dt>{t('home.seats')}</dt>
            <dd>{format(t('home.people'), { n: 様子.エージェント })}</dd>
          </div>
          <div>
            <dt>{t('home.rooms')}</dt>
    ...
    {:else}
      <h2>
        ...
        {#if エージェントを添える(相手)}<span class="seat">{エージェントの札(相手)}</span>{/if}
        <!-- **まだ誰もつながっていない行には出さない。**
             その行は「そういう仕組みがある」という案内であって、1 人ではない -->
        {#if 相手.種類 === '自分' || (相手.種類 === 'AI' && 相手.key !== この機械の印)}
      ...
      <!--
        **次に会うときの道を、いま書いておく**（オーナー・2026-09-11
        「再度やり取りする時に…案内すればいいかと。**案内の仕方が UI/UX 的に
        気になります**」）。

        **覚えないことは不具合ではない** ——
        「ずっと繋がっていたいわけではない場合もあるので」。
        悪いのは、**次にどうすればよいか画面が言わない**ことである。
      -->
      {#if 再会}
        <section class="reunion">
          <h4>{t('reunion.title')}</h4>
          <p>{t(再会.訳 as MessageKey)}</p>
          {#if 再会.道 === '鍵を渡し合う'}
            <div class="tail">
              <button type="button" class="quiet" onclick={() => 名付けを始める(相手)}>
                <Icon name="pencil" size={14} />{t('reunion.name')}
              </button>
            </div>
          {/if}
          <!-- **ルームの名前も、次に会うときの手がかりになる** -->
          <p class="hint">{t('reunion.room')}</p>
        </section>
      {/if}
      ...
      {#if 相手.種類 === 'AI' && 相手.key !== この機械の印 && 名乗り書き中 !== 相手.key}
      ...
      {#if 相手.種類 === '自分'}
        ...
        <section class="howto">
          ...
          <!-- **公開鍵とルームの鍵を取り違えさせない。**別のものである -->
          <p class="hint">{t('howto.key')}</p>
        </section>
      {/if}
      {#if 相手.種類 === 'ルーム'}
        <!-- **ルームは押して見るもの。**口は出さない（居るだけ） -->
        <p class="hint">
          {相手.いま会議に居る ? t('room.members.some') : t('room.alone')}
        </p>
        <!--
          **どの部屋に映像が付いているかを、開く前に出す**（**D112** / **#36**）。

          ASUS の実測（2026-09-15）——
          **2 つ建てたつもりがないのに部屋が 2 つあり、
          どちらに映像が付いているか画面のどこにも書いていなかった。**

          `映像の札()` は 09-16 に書いてあったが、**画面が値を持っていなかった**ので
          **一度も出ていなかった**（2026-09-18 に繋いだ）。
        -->
        {#if 相手.映像が付いている}
          <p class="hint strong"><Icon name="camera" size={14} />{t('room.hasvideo')}</p>
        {/if}
        ...
        <!--
          **ルームに名前を付けられる**（オーナー・2026-09-08
          「ルーム名も決められないと、どのルーム？って人間はなります」）。
          **名前はこの画面の中だけ** —— ルーム id はその場限りのものなので、
        ...
        <div class="rename">
          <input
            ...
            onkeydown={(e) => {
              if (e.key === 'Enter') ルームの名前を決める(相手);
            }}
          />
          <button type="button" class="quiet" onclick={() => ルームの名前を決める(相手)}>
            {t('profile.save')}
          </button>
        </div>
        <p class="hint">{t('room.name.hint')}</p>
        <!-- **抜ける口を置く。**入ったら出られないルームにしない -->
        <div class="tail">
  ...
  {#if 抜けるか確かめている && 相手 && ルームのid(相手.key)}
...
<style>
  ...
  */
  ...
  /* **どのエージェントか。**名乗りより弱く、しかし読める（取り違えを防ぐためのもの） */
  .seat {
    flex: none;
    font-size: var(--text-2xs-size);
    color: var(--text-tertiary);
