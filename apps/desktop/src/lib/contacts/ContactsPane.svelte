<script lang="ts">
  ...
  interface Props {
     ...
     * 自分の名前を書き換えられると、**同じPCの別のエージェントに化けられる。**
     */
  ...
  function 名(行: 行): string {
    ...
    if (名乗り) return 名乗り;
    if (行.種類 === '自分' || 行.key === このPCの印) return t(行.name as MessageKey);
  ...
  /** いまこのPCにつながっているエージェントが 1 人でも居るか。 */
  ...
  function 名乗りを引く(行: 行): ProfileRow | undefined {
    ...
    // このPCの行の key は `desk:<呼び方>`。**着いていないときの行（`desk:`）は素通し**
  ...
  function 顔を差し替えられるか(行: 行): boolean {
    return 行.種類 === '自分' || (行.種類 === 'AI' && 行.key !== このPCの印);
  }
    ...
    {#each 区画 as 一区画 (一区画.title)}
      ...
      {#each 一区画.行たち as 行 (行.key)}
        ...
        >
          ...
          <!-- **在エージェントは出さない。**着いているかどうかは、行が在ること自体で分かる -->
        </button>
      {/each}
    {/each}
  </div>
  ...
  <div class="person">
    ...
    {:else}
      <h2>
        ...
        {#if 相手.種類 === '自分' || (相手.種類 === 'AI' && 相手.key !== このPCの印)}
      ...
      {#if 相手.種類 === 'AI' && 相手.key !== このPCの印 && 名乗り書き中 !== 相手.key}
