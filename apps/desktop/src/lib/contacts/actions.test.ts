...
describe('選んだ相手に出す札（ダッシュボード・D49）', () => {
  ...
  it('マイ PC エージェント を「会議に呼ぶ」札は押せない（同じこのPCにつながっている）', () => {
  ...
  it('自分自身には、どの札も出さない', () => {
    const 自分: 相手 = { 種類: '自分', 住所を覚えている: true, いま会議に居る: true, このPCにつながっている: true };
...
describe('預かり所を置いているとき', () => {
  ...
  it('繋がっていないエージェントにも 4 枚出す（ぜんぶ「まだできない」）', () => {
    ...
    const 口たち = できること({ ...ai, このPCにつながっている: false });
