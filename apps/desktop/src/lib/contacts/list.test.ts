...
import { 連絡帳を組む, このPCの印, ルームのid, type 素材 } from './list';
...
const 素: 素材 = { 自分, このPCのAIたち: [], 会議の相手: [], 覚えた: [] };

describe('連絡帳の並び', () => {
  it('いちばん上は「この PC」（自分と、着いているエージェント）', () => {
    // `issues/012`「この PC で会議するとき、私とあなたはセットでしょっていう」
    const [先頭] = 連絡帳を組む({ ...素, このPCのAIたち: ['zumen のエージェント'] });
  ...
  it('誰も着いていなければ、まとめの行を作らない', () => {
    ...
    const [先頭] = 連絡帳を組む({ ...素, このPCのAIたち: [] });
    expect(先頭.行たち.some((r) => r.key === このPCの印)).toBe(false);
  ...
  it('着いているエージェントを、1 つずつ行にする', () => {
    ...
    // （2026-09-08 オーナー指摘）
    const [先頭] = 連絡帳を組む({ ...素, このPCのAIたち: ['zumen のエージェント', 'git-qa のエージェント'] });
  ...
  it('行ごとに別の印を持つ（押し分けられる）', () => {
    const [先頭] = 連絡帳を組む({ ...素, このPCのAIたち: ['a のエージェント', 'b のエージェント'] });
...
describe('留守中に届いた相手', () => {
  ...
  it('届いていなければ、区画そのものを出さない', () => {
    const 区画 = 連絡帳を組む({ 自分: 'ME', このPCのAIたち: [], 会議の相手: [], 覚えた: [] });
    expect(区画.find((s) => s.title === 'contacts.late')).toBeUndefined();
  });
});
...
describe('この PC のエージェント', () => {
  it('それぞれが 1 人として並ぶ', () => {
    ...
    const 区画 = 連絡帳を組む({
      自分: 'ME',
      このPCのAIたち: ['zumen のエージェント', 'git-qa のエージェント'],
      会議の相手: [],
      覚えた: [],
    });
