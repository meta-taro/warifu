...
/** 既定で許す口。**会話と、自分のエージェントの名乗りだけ**（受信箱も予定表も許さない）。 */
 ...
 * 待っている間そのエージェントを縛らない。
 *
 * **エージェントの名前をそのまま入れない**（`'` で囲んだ命令の中に入るため）——
 * 使えない字を落として、無ければ `agent` にする。
 */
export function 起こす(エージェント: string, 実体 = 'warifu'): string {
  const 安全 = エージェント.replace(/[^A-Za-z0-9_-]/g, '') || 'agent';
