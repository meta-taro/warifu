...
describe('初期画面のいまの様子', () => {
  ...
  it('この機械のエージェント・ルーム・覚えた相手を数える', () => {
    const 様子 = いまの様子(
      {
        ...空,
        この機械のAIたち: ['zumen', 'git-qa'],
        ルームたち: [
    ...
    expect(様子).toEqual({ エージェント: 2, ルーム: 2, ルームの人: 5, 覚えた: 1, 預かり所: true });
  });
...
describe('はじめの一歩', () => {
  ...
  it('この機械に誰か着いていれば出さない', () => {
    expect(はじめの一歩を出すか(いまの様子({ ...空, この機械のAIたち: ['zumen'] }, false))).toBe(false);
  });
