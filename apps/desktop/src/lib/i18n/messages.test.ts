...
describe('札から「ビデオ」を外した（**D108**・計画 段 B・2026-09-18）', () => {
  // オーナー（2026-09-14）——
  //   「**ここらへんが現状人は混乱します。ビデオ会議なのか、グループチャットなのか。**」
  //
  // **ビデオは行き先ではなく、ルームの中のスイッチである。**
  // 札に「ビデオ会議」と書いてある限り、**押せば会議が始まると読める** ——
  // 実際、押すとカメラが点いていた（2026-09-18 にオーナーが実物で踏んだ）。
  //
  // **札は 4 枚のまま**（**D88**）。`call` の中身を「ビデオ」から「呼ぶ」に変えた。
  const 札の鍵 = ['act.chat', 'act.group', 'act.call', 'act.calendar'] as const;
  const ビデオの言葉 = /ビデオ|[Vv]ideo|视频|영상/;

  for (const 言語 of LOCALES) {
    it(`${言語}: 4 枚の札に「ビデオ」が出ない`, () => {
      for (const 鍵 of 札の鍵) {
        expect(MESSAGES[言語][鍵]).not.toMatch(ビデオの言葉);
      }
    });

    it(`${言語}: 押す言葉にも「ビデオ」が出ない`, () => {
      // **`act.*.action` は押しボタンの字。**ここに残ると、札だけ直しても人は同じに読む
      for (const 鍵 of ['act.group.action', 'act.call.action'] as const) {
        expect(MESSAGES[言語][鍵]).not.toMatch(ビデオの言葉);
      }
    });
  }

  it('札は 4 枚のまま（**D88** を崩していない）', () => {
    expect(札の鍵).toHaveLength(4);
  });
});
