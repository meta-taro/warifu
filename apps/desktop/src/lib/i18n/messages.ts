...
export const MESSAGES: Record<Locale, Record<MessageKey, string>> = {
  ja: {
    ...
    'meeting.join.title': 'もらった鍵でルームに入る',
    'meeting.join.hint': 'もらった側が入ります。渡した側は待つだけです。',
    'meeting.join.action': 'ルームに入る',
    'meeting.key.own': 'これは自分のルームの鍵です。入ってほしい人に渡してください。',
    ...
    'chat.nobody': 'ルームにはまだ誰も入っていません。鍵を渡して、入ってもらうと送れます。',
    ...
    'mark.kept': '預けた',
    'mark.kept.hint': '相手が留守だったので、受け取り先へ封のまま置きました。届くのは相手が次に起動したときです。',
    'mark.none': '届かない',
    ...
    'howto.key': 'この手順で渡すのは「ルームの鍵」です。上の公開鍵ではありません。',
    ...
    'reunion.call': '連絡先から呼べます（鍵は要りません）。ただし**相手の側でも覚えていないと**、呼んでも通りません。',
    'reunion.wait': 'こちらからは呼べません（居場所を知りません）。相手から呼んでもらえば通ります。',
    'reunion.key': '初回と同じで、鍵を渡し合います。次から鍵なしにしたいなら、名前の隣の鉛筆で**呼び名を付けて**ください。',
  ...
  en: {
    ...
    'reunion.call': 'You can call them from Contacts (no key needed). It only goes through if **they remember you too**.',
    'reunion.wait': 'You cannot call them (their whereabouts are unknown). If they call you, it goes through.',
    'reunion.key': 'Same as the first time: you hand each other a key. To skip the key next time, **give them a name** with the pencil beside their name.',
  ...
  zh: {
    ...
    'reunion.title': '下次见面时',
    'reunion.call': '可以从联系人里呼叫（不需要钥匙）。但**对方也要记住你**，否则呼叫不会通。',
    'reunion.wait': '你无法呼叫对方（不知道对方在哪）。如果对方来呼叫你，就能通。',
    'reunion.key': '和第一次一样，互相交换钥匙。想让下次不需要钥匙，请用名字旁边的铅笔**起个称呼**。',
  ...
  ko: {
    ...
    'reunion.call': '연락처에서 부를 수 있습니다(키는 필요 없습니다). 다만 **상대도 기억하고 있어야** 연결됩니다.',
    'reunion.wait': '이쪽에서는 부를 수 없습니다(상대의 위치를 모릅니다). 상대가 부르면 연결됩니다.',
    'reunion.key': '처음과 같이 서로 키를 건넵니다. 다음부터 키 없이 하려면 이름 옆의 연필로 **이름을 붙여** 주세요.',
