...
export const MESSAGES: Record<Locale, Record<MessageKey, string>> = {
  ja: {
    ...
    'meeting.status.recvonly':
      'つながっています（相手の映像を受けています。あなたは送っていません —— ［映像と音を足す］を押すと送ります）',
    'meeting.status.sendonly':
    ...
    'home.group': 'この相手にルームキーを渡します。1 本につき 1 人です。押すと、ルームキーを出す所へ移ります。',
    'home.call': '覚えている住所へ直接つなぎます。同じ網なら繋がります。呼ぶだけで、カメラは点きません（映像はルームの中で足します）。',
    ...
    'pass.asked': 'エージェントが、これを使ってよいか聞いています',
    ...
    'room.mine': 'あなたが建てた',
    'room.back': '前のルームに戻る',
    'room.back.hint': '前に入ったルームへ入り直します。ルームキーをもう一度もらう必要はありません。ルームキーの期限（24 時間）が切れていても、相手が通してくれるなら戻れます。',
    ...
    'room.leave.do': '抜ける',
    'room.name.hint': '名前はこの PC に残ります（閉じても消えません）。相手には送りません。',
    ...
    'act.group': 'ルームに招く',
    'act.group.what': 'この相手にルームキーを渡します。1 本につき 1 人です。押すと、ルームキーを出す所へ移ります。',
    'act.group.action': 'ルームキーを出す',
    'act.group.desk.none': '繋がっていないエージェントは、ルームにも呼べません。',
    'act.call': 'ルームに呼ぶ',
    'act.call.net': '覚えている住所へ直接つなぎます。同じ網なら繋がります。相手が受け取る設定でなければ通りません。',
    'act.call.action': '呼ぶ（ルームキーは要りません）',
  ...
  en: {
    ...
    'meeting.status.recvonly':
      'Connected — you are receiving their video. You are not sending yours; press [Add video and sound] to send.',
    'meeting.status.sendonly':
    ...
    'home.group': 'Hands this person a room key. One key, one person. Pressing this takes you to where keys are issued.',
    'home.call': 'Connects straight to the address we remember. On the same network it connects. Calling alone does not turn your camera on — video is added inside the room.',
    ...
    'room.leave.do': 'Leave',
    'room.name.hint': 'The name is kept on this PC (closing the window does not lose it). It is not sent to anyone.',
    ...
    'act.group': 'Invite to a room',
    'act.group.what': 'Hands this person a room key. One key, one person. Pressing this takes you to where keys are issued.',
  ...
  zh: {
    ...
    'meeting.status.recvonly': '已连接（正在接收对方的视频。你没有发送自己的视频 —— 按［为这个房间加上影像］就会发送）',
    ...
    'home.group': '把一把 room key 交给这位对象。一把对一个人。按下后会转到发 room key 的地方。',
    'home.call': '直接连到我们记住的地址。同一个网络里可以连上。只是呼叫不会打开摄像头（影像在房间里才加上）。',
    ...
    'room.mine': '你建的',
    'room.back': '回到上一个房间',
    'room.back.hint': '重新进入上一个房间。不需要再要一把新的 room key —— 就算 room key 的 24 小时已经过了，只要对方还愿意放行，也能回去。',
    'room.leave': '退出这个 room',
    'room.leave.confirm': '要退出这个 room 吗？',
    'room.leave.hint': 'room 里的所有人都会知道你退出了。这里的会话不会保留。要再进来需要重新拿到room key。',
    'room.leave.do': '退出',
    'room.name.hint': '名字保存在这台电脑上（关闭窗口也不会消失）。不会发送给别人。',
    ...
    'act.group': '邀请进入房间',
    'act.group.what': '把一把 room key 交给这位对象。一把对一个人。按下后会转到发 room key 的地方。',
  ...
  ko: {
    ...
    'meeting.status.recvonly':
      '연결되어 있습니다 (상대의 영상을 받고 있습니다. 당신은 보내지 않았습니다 —— ［이 룸에 영상을 더하기］를 누르면 보냅니다)',
    'meeting.status.sendonly':
    ...
    'home.group': '이 상대에게 룸 키를 건넵니다. 한 개에 한 사람입니다. 누르면 룸 키를 발급하는 곳으로 이동합니다.',
    'home.call': '기억하고 있는 주소로 바로 연결합니다. 같은 망이면 연결됩니다. 부르기만 해서는 카메라가 켜지지 않습니다（영상은 룸 안에서 더합니다）.',
    ...
    'room.back.hint': '이전 룸에 다시 들어갑니다. 키를 다시 받을 필요는 없습니다. 룸 키의 24 시간이 지나도, 상대가 들여보내 준다면 돌아갈 수 있습니다.',
    ...
    'room.leave.do': '나가기',
    'room.name.hint': '이름은 이 PC에 남습니다 (닫아도 사라지지 않습니다). 상대에게는 보내지 않습니다.',
    ...
    'act.group': '룸에 초대',
    'act.group.what': '이 상대에게 룸 키를 건넵니다. 한 개에 한 사람입니다. 누르면 룸 키를 발급하는 곳으로 이동합니다.',
