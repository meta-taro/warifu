<script lang="ts">
  ...
  import { このPCの印, ルームのid, type 行 as 連絡帳の行 } from '$lib/contacts/list';
  ...
  /**
   * このPCにつながっている人数（同じ PC の AI）。
   *
   * **「相手が居ない」と「話し相手が 1 人も居ない」は違う。**
   * オーナーが「会議ありきのチャットじゃない」と言った所である（2026-09-06）。
   */
  let このPCのAIたち = $state<string[]>([]);
  const このPCの人数 = $derived(このPCのAIたち.length);
  /**
   ...
   * このPCのエージェントと 1 行話しただけで「待っている」になり、
   ...
   */
  const 宛先 = $derived(宛先を決める(選んだ相手, このPCのAIたち));
  ...
  const 選んだエージェント = $derived(
    選んだ相手 && 選んだ相手.startsWith(このPCの印) && 選んだ相手 !== このPCの印
      ? 選んだ相手.slice(このPCの印.length)
      : null,
  );
  const 選んだエージェントがつながっているか = $derived(!!選んだエージェント && このPCのAIたち.includes(選んだエージェント));
  ...
  const エージェントの名前たち = $derived(
    Object.fromEntries(
      名乗りたち
        .filter((p) => p.who.startsWith(このPCの印) && p.name)
        .map((p) => [p.who.slice(このPCの印.length), p.name]),
    ),
  );
  ...
  const 届く先がある = $derived(
    ...
    送り方.種類 === '預ける' || 送れるか({ 相手: remotes.length, このPCの人数 }),
  );
  ...
  const 連絡帳の素材 = $derived({
    ...
    名乗りのあるエージェント: 名乗りたち
      .filter((p) => p.who.startsWith(このPCの印))
      .map((p) => p.who.slice(このPCの印.length)),
    ルームたち,
    ルームの名前: ルームの名前たち,
  ...
  /** いま見ているルーム。**同じ PC の AI を選んでいればこのPCのルーム。** */
    ...
    void (async () => {
      ...
      unsubs.push(
        await onEvent<string>(EVENT_JOINED, async (key) => {
          ...
          // **このPCとルームの両方へ**（`issues/1`）—— 入退室も、どちらの入口でも読める
          ルームの会話たち = このPCとルームへ足す(ルームの会話たち, いまのルーム, {
      ...
      unsubs.push(
        // **このPCにつながったエージェントが、自分で名乗った。**画面にもすぐ出す
  ...
  $effect(() => {
    const 状態 = [
      ...
      `相手 ${remotes.length} 人`,
      // **このPCに AI が着いているかは、後から追えないと分からない。**
      // 「送れない」と言われたときに、居たのか居なかったのかが読めなくなる
      `このPC ${このPCの人数} 人${このPCのAIたち.length ? `（${このPCのAIたち.join('・')}）` : ''}`,
   ...
   *
   * このPCは `名前（名乗り）` の形で渡してくる（`profile::エージェントの名札`・D77）。
  ...
  async function 話す(body: string) {
    ...
    try {
      ...
      log(話の記録('送信', 宛先 ?? (会議中 ? `${remotes.length} 人` : 'このPC'), body));
      ...
      if (送り方.種類 === 'このPC') {
        ...
        // ルームにもこのPCぜんぶにも積まない —— **届いていない所に出すのは嘘である**
        ルームの会話たち = エージェントへ足す(ルームの会話たち, 送り方.宛先, 私の行);
      } else {
        // **このPCとルームの両方へ積む**（`issues/1`）。自分の行だけがルームに残り、
        // エージェントの行がこのPCにしか無い、という食い違いをなくす
        ルームの会話たち = このPCとルームへ足す(ルームの会話たち, いまのルーム, 私の行);
      }
    } catch (e) {
      notice = 読める(e);
    }
  ...
  async function 連絡帳から押す(種類: 口の種類, 相手: 連絡帳の行) {
    ...
    // このPCのエージェントは同じエージェントに居る。**繋ぎに行くものが無い**
    if (相手.key === このPCの印) return;
  ...
  const 顔を差し替えられる先 = $derived.by(() => {
    ...
    if (選んだ相手.startsWith(このPCの印) && 選んだ相手 !== このPCの印) return 選んだ相手;
    return null;
  });
  /** いま落とせる所にドラッグしているか。 */
