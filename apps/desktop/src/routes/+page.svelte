<script lang="ts">
  ...
  /**
   * いま選んでいる席（`desk:<席>`）。席の行でなければ `null`。
   *
   * **切れている席は宛先が決まらない**ので、`送り方` は部屋あてになる ——
   * それだけを見ると「ルームに誰も入っていません」と出てしまう
   * （2026-09-10 に実物で出た）。**選んでいる行を見て、席の話をする。**
   */
  const 選んだ席 = $derived(
    選んだ相手 && 選んだ相手.startsWith(机の印) && 選んだ相手 !== 机の印
      ? 選んだ相手.slice(机の印.length)
      : null,
  );
  const 選んだ席が着いているか = $derived(!!選んだ席 && 机のAIたち.includes(選んだ席));
