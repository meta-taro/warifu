        ...
        loop {
            tokio::select! {
                来た = 口.受ける() => {
                    ...
                    if 応じる(&app, &行, &mut 口).await.is_err() {
                        break;
                    }
                }
                配られた = 聞く.recv() => {
...
async fn 応じる(
    ...
    match 中身 {
        ...
        ToDesk::Listen => Ok(()),
        ToDesk::Say { body } => {
            let 出た = 流す(app, &body).await;
            if let Err(訳) = 出た {
                let _ = 口.送る(&FromDesk::Nobody.書く()).await;
...
async fn 流す(app: &AppHandle, body: &str) -> Result<(), String> {
    ...
    // 机に着いている他のエージェントにも同じ行を見せる
    let _ = bridge.desk.send(FromDesk::Heard {
