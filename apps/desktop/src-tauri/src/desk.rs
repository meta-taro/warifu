...
fn 席に着ける(席: u64, 呼び方: &str) {
    let mut 棚 = 席たち.lock().expect("毒されていない");
    let 使われている =
        |名: &str, 棚: &BTreeMap<u64, String>| 棚.iter().any(|(番, 呼)| *番 != 席 && 呼 == 名);
    if !使われている(呼び方, &棚) {
        棚.insert(席, 呼び方.to_owned());
        return;
    }
    for n in 2..=99u32 {
        let 候補 = format!("{呼び方} {n}");
    ...
    // **ここまで来たら席番号で分ける。**同じ名前で潰すよりよい
    棚.insert(席, format!("{呼び方}（{席}）"));
}
...
fn 座らせる(app: AppHandle, 一本: impl warifu_desk::一本) {
    tauri::async_runtime::spawn(async move {
        ...
        // **名乗るまで席に着けない**（オーナー・2026-09-08
        // 「名乗らないとリストに出ない仕様でもいいですね。
        //   ターミナルからいちど割符に入るように案内をして」）。
        //
        // **どこで動いているか分からない席を、一覧に出さない。**
        // 出しても「どの階層のエージェントか不明」で、押しても意味が無い。
        // 着けるのは `ToDesk::Listen { 場所: Some(..) }` が来たときだけ。
        loop {
            tokio::select! {
                来た = 口.受ける() => {
    ...
    match 中身 {
        ...
        ToDesk::Listen { 場所 } => {
            ...
            *名乗り = Some(名.clone());
            ...
            席の数を伝える(app);
            // **いつ着いたかを返す**（`issues/4`）。
            // これが無いと、席から「届いていない」と「着く前だった」を見分けられない
            let who = 席たち
                .lock()
                .expect("毒されていない")
                .get(&自分)
...
async fn 流す(
    ...
    // **同じ席が 2 つの名前で出る** —— 人はそれを 2 人だと読む。
    let 名札 = crate::profile::席の名札(&呼び方);
...
fn 届き方(id: u64) -> (Vec<String>, Vec<String>) {
    ...
    let 読んだ: Vec<String> = 席棚
        .iter()
        .filter(|(席, 名)| 届いた.contains(名) && 読んだ棚.get(*席).copied().unwrap_or(0) >= id)
        .map(|(_, 名)| 名.clone())
        .collect();
...
mod tests {
    ...
    fn 先に居るほうの名前は変えない() {
        ...
        let _順 = 一つずつ();
        席に着ける(10, "zumen のエージェント");
        席に着ける(11, "zumen のエージェント");
        assert_eq!(いまの呼び方(10), "zumen のエージェント");
        assert_eq!(いまの呼び方(11), "zumen のエージェント 2");
    }

    #[test]
    fn 同じ席が名乗り直しても番号は付かない() {
        // `Listen` は着いた後に来る。**自分自身と重なったことにしない**
        let _順 = 一つずつ();
        席に着ける(20, &呼び方(None));
        席に着ける(20, "zumen のエージェント");
        席に着ける(20, "zumen のエージェント");
