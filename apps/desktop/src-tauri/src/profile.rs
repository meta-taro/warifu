...
//!
//! オーナー指示（2026-09-08）——
//! 「**この機械のAIもまた一人の人としてプロフィールを持てるようにします。
//!   エージェントも人です。その管理はこの機械の持ち主が編集可能とします。**」
//!
//! # 書き換えられるのは、この端末の持ち主だけ
//!
//! **AI が自分の名前を書き換える口は作らない。**
//! 作ると、**同じ機械の別のエージェントに化けられる**（`chat_send` はエージェントの名乗りを載せるので、
...
/// **この機械につながったエージェントが、自分のエージェントのプロフィールを書く。**
///
/// オーナー指示（2026-09-08）——
/// 「**エージェントが MCP で接続できたらエージェント自身に
/// プロフィールをかけるようにしておくと、楽だとおもいます。**」
///
/// **書けるのは自分のエージェントだけ。**どのエージェントかは**繋いできた口で決まる**（`desk.rs`）ので、
/// ここへ「誰の」は渡ってこない。
///
...
/// **そのエージェントの名乗り**を、線に載せる形にする。
///
/// 名乗っていれば `図面くん（zumen）`、名乗っていなければエージェントそのまま。
/// **どこのエージェントかを落とさない** —— 落とすと、相手の画面で取り違えられる（**D75**）。
#[must_use]
pub fn エージェントの名札(呼び方: &str) -> String {
    let Ok(面々) = 読む() else {
        return 呼び方.to_owned();
    };
    let 名前 = 面々
...
/// **会話と名簿で同じエージェントが別の名前で出る**（2026-09-09 に実物で見た ——
