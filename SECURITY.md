...
| `room-secret.tsv` | **The room passphrase** (**D118**, 32 bytes per room). **Anyone holding it can let anyone else into that room.** It is handed only to peers who already passed the door, over that connection — it is **never carried in the key** (`WARIFU1-…`), because keys get pasted in public. Deleted when you leave the room. |
