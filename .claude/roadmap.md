...
| フルメッシュの実用上限 | **3〜4 人**（帯域が `K×(N−1)`）→ 当初 4 人固定（D9）→ **2026-09-02 に会議ごとの定員へ変更**（既定 **12** / 外枠 16・**D27**） |
...
| **M3** | Intent の口 — `file.*` / `meeting.*` を M2 の上に載せる | 単体テスト + 2 プロセス | 要 | **済**（D14） |
...
## Step 3.5 — **複数人の会議**（2026-08-17 新設・`issues/005`）

D9。**SFU を使わないフルメッシュに限る。**中継を伴う構成は最後尾のまま（D7 未決）。

**当初は「4 人まで」だったが、2026-09-02 に会議ごとの定員へ変えた**（既定 **12** / 外枠 **16**・**D27**）。
切る線は**人数ではなく中継の有無**である（5 人フルメッシュは D7 に触れない）。

| | 作るもの | 状態 |
|---|---|---|
| **M4** | SDP / ICE を **M2 の E2EE チャネル上で**交換する（外部のシグナリングサーバを使わない） | **済**（D15） |
| **M5-a** | **器**（Tauri 2 + SvelteKit・自作タイトルバー・トークン・多言語・更新の判定） | **済**（2026-09-02・D37・26 件 green） |
| **M5-b** | **経路と名簿**（WebRTC の統計 → 直接/中継/不明・定員の見え方） | **済**（2026-09-02・41 件 green） |
| **M5-c1** | **会議の進行**（名簿の動き・誰が offer を出すか）。`warifu-app`・機材不要 | **済**（2026-09-02・D38・12 件 green） |
| **M5-c2** | Tauri 2 + WebView の WebRTC で 2 人の映像・音声。**Codec は書かない** | **書けた**（2026-09-02）。**映像が出るかは人が 2 台で確かめる**（`docs/trial.md`） |
| **M6** | 3〜4 人 →**既定 12 人**のフルメッシュ。**参加者ごとに帯域と CPU を実測して記録する** | |
| **M7** | 画面共有（会議中に md-business の文書を見せる用途がこれ） | |

**M6 の実測が、この Step の主目的である。**
既定 12 も外枠 16 も文献値であって、**手元の回線で成立するかは測らないと分からない**。
成立しなければ**崩れた人数まで既定を下げる**か、画質（`K`）を落とす。**SFU には進まない**（D27）。
...
| Phase | 内容 | v2 の対応章 | 備考 |
|---|---|---|---|
| 2 | P2P Text / Connection Request / Rate Limit / Spam Defense / **File Quarantine** | v2 §11 §12 | **Relay 自社運用はここまで行わない**（D3）。**File Quarantine は v2 で新規**（受信 File を Downloads へ直接置かず検査。**Trusted からの File も Zero Trust**）。**2026-09-02: File Quarantine は `warifu-quarantine` として動いている**（20 件 green・**D30**）。**2026-09-02: Connection Request / Rate Limit / Spam Defense も `warifu-door` として動いている**（14 件 green・**D31**）。残りは P2P Text（`warifu-intent` に `text.*` の口を足すだけ） |
| **3** | **Agent Identity / Capability / Approval Gate / A2A Intent / MCP Adapter**（**2026-09-02 着手・関所は動いている**） | v2 §15 §16 **§17** | **D5 を実装で担保する所。**代表 Demo は **v2 §17 の AI 会議調整**（`meeting.request` → 空き時間だけ返却 → 双方承認 → 双方 Calendar へ Event）。**Private Calendar の中身を相手に見せない**制約が Capability の実地テストになる。**D5（原則）の下に、2026-09-01 企画書 §24 の実装項目を置く**（HTML sanitize / remote image blocking / URL isolation / instruction-data separation / allowlist tools / permission scopes / signature verification / **モデルから直接 tool を呼ばせない** / policy engine 経由）。`attachment sandbox` は Phase 2 の File Quarantine と同一なので重複させない。**D5 は書き換えない**（PRD §12-2）。**判定をモデルの外に出す所は `warifu-capability` として動いている**（15 件 green・**D24** / `issues/008`）。**MCP Adapter も `warifu-mcp` として動いている**（10 件 green・**D25**・実物のやり取りで確認済み）。**代表 Demo（v2 §17 の会議調整）も `warifu-calendar` として動いている**（19 件 green・**D26**）。残りは MCP から実際の Calendar サービスへ繋ぐ所 |
| **3-b** | **低トークン読み取り層**（**2026-09-01 着手・R1〜R5 済／実サーバ未接続**）（Metadata First / Progressive Disclosure Level 0〜4 / **Deterministic Parser First** / Thread Compression / Deduplication / token 会計 / **Action Inbox**） | 2026-09-01 企画書 §11〜§14 §38 §39 | **Phase 3 と独立に着手できる。**P2P も Directory も相手も要らず **n=1 で効く**唯一の層で、既存のメール（SMTP / IMAP）に対してだけでも動く＝ PRD §2 とそろう。**Adapter（Phase 5）の内側に書かない** — 経路ごとに同じものを作り直すことになる。詳細は PRD §12-1 / `issues/007`。**R1（既定と段）・R2（規則）・R3（会計と保存）・R4（重複除去）は `warifu-read` / `warifu-imap` として動いている**（77 件 green・**D18**〜**D23**）。残りは**人が資格情報を入れて実際に回す工程**だけ（`--example inbox`）（`issues/007` の「進み方」） |
| ~~4~~ | ~~File / Voice / Video / Screen Share~~ | v2 §13 | **2026-08-17 に Step 3.5 / 3.6 へ前倒し**（D9）。ここには残らない |
| 5 | Email / Messenger / Telephone Adapter | v2 §25 Phase 8 / 2026-09-01 企画書 §15 §16 | **2026-09-01 企画書の SMTP / IMAP 互換・Postfix の位置づけはここに入る**（PRD §12）。「決定済み」の**Email を最初に作らない**は変えない。**共通ドメイン ＋ 中央 Directory（企画書 §5 §6）は採らない**（PRD §12-3・D2 の選択肢 e へ変換） |
