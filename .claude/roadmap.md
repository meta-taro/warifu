...
| **3-b** | **低トークン読み取り層**（**2026-09-01 着手・R1〜R4 済**）（Metadata First / Progressive Disclosure Level 0〜4 / **Deterministic Parser First** / Thread Compression / Deduplication / token 会計 / **Action Inbox**） | 2026-09-01 企画書 §11〜§14 §38 §39 | **Phase 3 と独立に着手できる。**P2P も Directory も相手も要らず **n=1 で効く**唯一の層で、既存のメール（SMTP / IMAP）に対してだけでも動く＝ PRD §2 とそろう。**Adapter（Phase 5）の内側に書かない** — 経路ごとに同じものを作り直すことになる。詳細は PRD §12-1 / `issues/007`。**R1（既定と段）・R2（規則）・R3（会計と保存）・R4（重複除去）は `warifu-read` として動いている**（59 件 green・**D18**〜**D21**）。残りは R5（IMAP と添付）と、人が受信箱で測る工程（`issues/007` の「進み方」） |
