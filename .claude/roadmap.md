...
| **3** | **Agent Identity / Capability / Approval Gate / A2A Intent / MCP Adapter**（**2026-09-02 着手・関所は動いている**） | v2 §15 §16 **§17** | **D5 を実装で担保する所。**代表 Demo は **v2 §17 の AI 会議調整**（`meeting.request` → 空き時間だけ返却 → 双方承認 → 双方 Calendar へ Event）。**Private Calendar の中身を相手に見せない**制約が Capability の実地テストになる。**D5（原則）の下に、2026-09-01 企画書 §24 の実装項目を置く**（HTML sanitize / remote image blocking / URL isolation / instruction-data separation / allowlist tools / permission scopes / signature verification / **モデルから直接 tool を呼ばせない** / policy engine 経由）。`attachment sandbox` は Phase 2 の File Quarantine と同一なので重複させない。**D5 は書き換えない**（PRD §12-2）。**判定をモデルの外に出す所は `warifu-capability` として動いている**（15 件 green・**D24** / `issues/008`）。**MCP Adapter も `warifu-mcp` として動いている**（10 件 green・**D25**・実物のやり取りで確認済み）。残りは Calendar など具体的な相手 |
